...
function boot(){
 ...
 fetch(b+"/api/distribution/feed?"+q,{mode:"cors",credentials:"omit",cache:"no-store"}).then(function(r){if(!r.ok)throw 0;return r.json()}).then(function(d){var items=d.items||[];grid.innerHTML=items.length?items.map(function(i){return'<a class="item" href="'+esc(i.url)+'" target="_blank" rel="noopener"><img class="thumb" src="'+esc(i.thumbnail)+'" alt="" loading="lazy"><div><div class="title">'+esc(i.title)+'</div><div class="meta"><span>'+esc(i.channel)+'</span><span class="badge">'+esc(i.tier||"DISCOVERY")+'</span><span>'+esc(i.topic||"All")+'</span></div></div></a>'}).join(""):'<div class="empty">No RALLIVIO discovery content is available for this filter yet.</div>'}).catch(function(){grid.innerHTML=""})}
