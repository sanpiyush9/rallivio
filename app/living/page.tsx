...
export default function LivingDiscover() {
  ...
  const command = (s: string) => {
    ...
    if (c) { setFilter(c.name); setQ(""); setNotice(`RALLIVIO tuned the field to ${c.name}.`); return; }
    const p = platforms.find(x => l.includes(x.name.toLowerCase()));
    if (p) { setActivePlatform(p.name); setNotice(`${p.name} is now the active environment.`); return; }
    if (l.includes("creator")) { go("/creators"); return; }
    if (l.includes("brand")) { go("/brands"); return; }
    if (l.includes("opportun")) { go("/opportunities"); return; }
    setQ(s); setFilter("Trending"); setNotice(`Searching the verified discovery pool for “${s}”.`);
  };
  ...
  const activatePlatform = (p: typeof platforms[number]) => { setActivePlatform(p.name); setNotice(p.connected ? "YouTube is source-connected. The field is responding to verified observations." : `${p.name} environment is active. Its source adapter is not connected yet, so no activity is fabricated.`); };

  return <main className="rv" onPointerMove={move} onPointerLeave={() => setPointer({ x: 0, y: 0 })}><style>{css}</style>
      ...
      <div className="ecosystem">
        <div className="field" aria-label="RALLIVIO living platform field">
          <div className="fieldInner" style={{ transform: `perspective(1100px) rotateY(${pointer.x * -4}deg) rotateX(${pointer.y * 3}deg)` }}>
            ...
            {platforms.map((p, i) => <button key={p.name} className={`platform p${i} ${activePlatform === p.name ? "selected" : ""}`} type="button" onClick={() => activatePlatform(p)} onPointerEnter={() => setActivePlatform(p.name)}><span>{p.icon}</span><b>{p.name}</b><small>{p.connected ? "Connected" : "Explore"}</small></button>)}
            <button className="core" type="button" onClick={() => { setActivePlatform("YouTube"); setFilter("Trending"); setQ(""); setNotice("RALLIVIO re-centered. The living field is listening."); }}><div className="corePulse"/><strong>RALL<span>IVIO</span></strong><small>LIVING DISCOVERY SYSTEM</small><i><b>●</b> {loading ? "syncing" : `${ranked.length} verified signals`} {activePlatform !== "YouTube" ? `· ${activePlatform} focus` : "· in motion"}</i></button>
          </div>
        </div>
        <div className="fieldHint"><span>◉</span> Touch a platform or the RALLIVIO core to shift the field</div>
      </div>
    </section>
