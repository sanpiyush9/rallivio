...
const platforms: Platform[] = [
  ...
  { id: "snapchat", name: "Snapchat", kind: "snapchat", x: 68.5, y: 87.0, connected: false },
  ...
  { id: "facebook", name: "Facebook", kind: "facebook", x: 9.5, y: 51.0, connected: false },
...
const themeDefinitions = [
  { id: "nebula", name: "Royal Indigo", short: "Indigo", desc: "indigo / electric blue", mode: "SIGNATURE", accent: "indigo" },
  { id: "aurora", name: "Deep Ocean", short: "Ocean", desc: "teal / cobalt", mode: "IMMERSIVE", accent: "ocean" },
  { id: "neon", name: "Copper Ember", short: "Ember", desc: "copper / coral", mode: "ENERGY", accent: "ember" },
...
export default function LivingDiscover() {
  ...
  const [showAudienceMethod, setShowAudienceMethod] = useState(false);
  ...
  useEffect(() => {
    const savedTheme = window.localStorage.getItem("rallivio-theme-v2");
    if (savedTheme && ["nebula", "aurora", "neon", "lunar"].includes(savedTheme)) setTheme(savedTheme);
  }, []);

  useEffect(() => {
    window.localStorage.setItem("rallivio-theme-v2", theme);
  }, [theme]);
  ...
  return <main className={`rv theme-${theme}`}>
    ...
    <header className="topbar">
      ...
      {userEmail ? (
        <button className="loginButton avatarButton profileButton" type="button" aria-label="Open profile" onClick={() => router.push("/account")} title={userName || userEmail}>
          {userAvatar ? <img src={userAvatar} alt="" /> : <span>{(userName || userEmail).charAt(0).toUpperCase()}</span>}
        </button>
      ) : (
        <button className="loginButton loginTextButton" type="button" aria-label="Login" onClick={() => router.push("/login")}>Login</button>
      )}
      </div>
    </header>
    ...
    <section className="radarSection">
      <div className="radarPanel">
        <div className="radarPanelHead"><div><h3>What’s Happening Now</h3><p>{apiUsageLatestAt ? "Current size of each topic in the verified discovery pool" : "Awaiting first acquisition"}</p></div><span className="scanState">{apiUsageLatestAt ? <><i/> LIVE SNAPSHOT</> : "Awaiting first acquisition"}</span></div><div className="discoveryMethodNote">ⓘ Signals = verified YouTube observations · counts show the current pool · captured from source stats timestamps</div>
        <div className={`radarVisual liveRadar ${apiUsageLatestAt ? "" : "radarIdle"}`}>{apiUsageLatestAt ? <><div className="radarHud"><span>DISCOVERY FIELD</span><b>LIVE</b></div><div className="radarGrid"/><div className="radarSweep"/><div className="radarRings"><i/><i/><i/><i/><i/><i/><b/></div><div className="radarCrosshair"/><div className="radarTarget t1"><i/><b>01</b></div><div className="radarTarget t2"><i/><b>02</b></div><div className="radarTarget t3"><i/><b>03</b></div><div className="radarTarget t4"><i/><b>04</b></div><div className="radarTarget t5"><i/><b>05</b></div><div className="radarCenter"><i/></div><div className="radarRange r1">25%</div><div className="radarRange r2">50%</div><div className="radarRange r3">75%</div></> : <span className="radarEmpty">No acquisition activity yet.</span>}</div>
      ...
      <div className="spotlightPanel">
        ...
        {spotlightCreators.length > 0 && <div className="audienceAudit">
          <button type="button" className="audienceAuditToggle" onClick={() => setShowAudienceMethod(v => !v)} aria-expanded={showAudienceMethod}>
            <span>ⓘ How RALLIVIO uses this score</span><b>{showAudienceMethod ? "Hide" : "Explain"}</b>
          </button>
          {showAudienceMethod && <div className="audienceAuditBody">
            <div><b>WHAT IT IS</b><span>A source-provided audience comparison score attached to the creator observation.</span></div>
            <div><b>HOW WE USE IT</b><span>RALLIVIO uses the score to sort the verified creator list, so creators with higher source-provided comparison values appear first.</span></div>
            <div><b>WHAT THE NUMBER MEANS</b><span>The value is a comparison score, not a multiplier. A score of 3.25 does <strong>not</strong> mean 3.25× the audience.</span></div>
            <div><b>NOT THIS</b><span>It is not subscriber count, a percentage, views, reach, or a prediction of future audience growth. Subscriber count is shown separately.</span></div>
            <div><b>FORMULA</b><span>The source data provides the score, but its underlying formula is not currently documented in RALLIVIO’s data contract. RALLIVIO therefore does not invent a conversion or claim what 3.25 represents beyond the source comparison value.</span></div>
          </div>}
        </div>}
...
.topbar{padding:13px 38px;background:rgba(5,7,18,.82);border-bottom-color:rgba(255,255,255,.09);box-shadow:0 8px 40px rgba(0,0,0,.22);display:grid!important;grid-template-columns:240px minmax(0,1fr) 300px auto;align-items:center;gap:22px!important}
.brand{font-size:30px;transition:transform .2s ease}.brand:hover{transform:translateY(-1px)}
.topbar nav{display:flex!important;align-items:center!important;justify-content:center!important;gap:26px!important;min-width:0!important;overflow:hidden!important}.topbar nav button{position:relative;color:#b9b8ca;transition:color .2s,background .2s;flex:0 0 auto!important}.topbar nav button:nth-child(3){max-width:185px;overflow:hidden;text-overflow:ellipsis}.topbar nav button:after{content:"";position:absolute;left:14px;right:14px;bottom:4px;height:2px;border-radius:3px;background:var(--accent);transform:scaleX(0);transition:transform .2s}.topbar nav button:hover:after,.topbar nav button.active:after{transform:scaleX(1)}
.search{width:100%!important;min-width:0!important;margin:0!important;background:rgba(255,255,255,.035);border-color:rgba(255,255,255,.1);box-shadow:inset 0 1px 0 rgba(255,255,255,.04);overflow:hidden!important}.search input{min-width:0!important;width:100%!important}.search button{flex:0 0 48px!important}
...
.themeScrim{display:none}.themePickerWrap{position:relative;z-index:80}.themeButton{min-width:auto;height:38px;display:flex;align-items:center;gap:7px;padding:0 12px 0 10px;background:linear-gradient(135deg,#18142f,#221a3e);border-color:#ffffff22;box-shadow:inset 0 0 18px #ffffff05,0 8px 25px #0003}.themeButton:hover,.themeButton[aria-expanded="true"]{border-color:var(--accent)88;box-shadow:0 0 22px var(--glow)33,inset 0 0 18px #ffffff07}.themeButton>span:not(.themeButtonGlyph){font-size:10px;letter-spacing:.15px}.themeButtonGlyph{font-size:13px!important;color:var(--accent);text-shadow:0 0 12px var(--glow)}.themeButtonDot{width:6px;height:6px;border-radius:50%;background:var(--accent);box-shadow:0 0 10px var(--glow);margin-left:1px}.themeMenu{position:absolute;right:0;top:48px;width:318px;padding:12px;border:1px solid #ffffff18;border-radius:20px;background:linear-gradient(145deg,#0c0f22f5,#080a17f8);box-shadow:0 28px 80px #000b,0 0 0 1px #0008;backdrop-filter:blur(26px) saturate(145%);-webkit-backdrop-filter:blur(26px) saturate(145%);z-index:90;overflow:hidden}.themeMenu:before{content:"";position:absolute;inset:0;pointer-events:none;background:radial-gradient(circle at 12% 0%,var(--glow)18,transparent 34%),linear-gradient(120deg,#ffffff08,transparent 42%)}.themeMenu>*{position:relative}.themeMenuHead{display:flex;align-items:center;justify-content:space-between;padding:3px 3px 11px;border-bottom:1px solid #ffffff0c}.themeMenuHead div{display:grid;gap:3px}.themeMenuHead small,.themeMenuLabel{color:#6f748f;font-size:7px;font-weight:900;letter-spacing:1.5px}.themeMenuHead b{font-size:12px;letter-spacing:-.1px}.themeMenuHead>span{display:flex;align-items:center;gap:5px;color:#6fe0ae;font-size:7px;font-weight:900;letter-spacing:.8px}.themeMenuHead>span i,.themeMenuFoot i{width:5px;height:5px;border-radius:50%;background:#6fe0ae;box-shadow:0 0 9px #6fe0ae;display:inline-block}.themeCurrent{display:flex;align-items:center;gap:10px;margin:11px 0 13px;padding:10px;border:1px solid var(--accent)35;border-radius:14px;background:linear-gradient(135deg,var(--accent)0d,#ffffff03)}.themeCurrentGlow{width:34px;height:34px;flex:0 0 34px;border-radius:11px;border:1px solid var(--accent)55;box-shadow:0 0 24px var(--glow)55;position:relative;overflow:hidden;background:radial-gradient(circle at 28% 25%,#fff8,transparent 22%),linear-gradient(135deg,var(--accent),var(--accent2))}.themeCurrentGlow:after{content:"";position:absolute;inset:8px;border:1px solid #fff7;border-radius:50%;opacity:.7}.themeCurrent div{display:grid;gap:2px}.themeCurrent small{color:#777b99;font-size:6px;letter-spacing:1.1px;font-weight:900}.themeCurrent strong{font-size:10px}.themeCurrent em{font-style:normal;color:#8b90aa;font-size:7px}.themeMenuLabel{padding:0 2px 7px}.themeOptions{display:grid;gap:6px}.themeOption{position:relative;width:100%;display:grid;grid-template-columns:54px 1fr auto 15px;gap:10px;align-items:center;text-align:left;border:1px solid transparent;border-radius:14px;background:#ffffff02;padding:8px}.themeOption:hover,.themeOption.active{background:#ffffff08;border-color:#ffffff14}.themeOption.active{box-shadow:inset 2px 0 0 var(--accent),0 0 24px var(--glow)0d}.themePreview{position:relative;width:54px;height:38px;border-radius:10px;overflow:hidden;border:1px solid #ffffff1f;box-shadow:inset 0 0 14px #ffffff09,0 0 16px #0005}.themePreview:before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 70% 25%,#fff8 0 3%,transparent 4%),linear-gradient(135deg,var(--preview-a),var(--preview-b) 55%,#080b18)}.themePreview:after{content:"";position:absolute;width:24px;height:24px;left:15px;top:7px;border:1px solid #fff8;border-radius:50%;box-shadow:0 0 15px var(--preview-glow)}.themePreview i,.themePreview b,.themePreview em{position:absolute;border-radius:50%;display:block}.themePreview i{width:3px;height:3px;left:8px;top:10px;background:#fff;box-shadow:36px 4px #fff8,12px 20px #fff7}.themePreview b{width:6px;height:6px;right:7px;bottom:7px;background:var(--preview-glow);box-shadow:0 0 10px var(--preview-glow)}.themePreview em{width:14px;height:14px;left:20px;top:12px;border:1px solid #fff9}.themePreview.nebula{--preview-a:#5630e8;--preview-b:#c869ff;--preview-glow:#9b62ff}.themePreview.aurora{--preview-a:#087fa6;--preview-b:#52d89d;--preview-glow:#42e6c0}.themePreview.neon{--preview-a:#c51b71;--preview-b:#ff9b4a;--preview-glow:#ff62c6}.themePreview.lunar{--preview-a:#536987;--preview-b:#e5f5ff;--preview-glow:#a9dcff}.themeOptionCopy{display:grid;gap:3px;min-width:0}.themeOptionCopy b{font-size:10px}.themeOptionCopy small{font-size:7px;color:#777b99;white-space:nowrap}.themeMode{padding:4px 6px;border:1px solid #ffffff10;border-radius:6px;color:#717690;font-size:6px;text-transform:uppercase;letter-spacing:.7px}.themeCheck{font-size:11px;font-weight:900;color:var(--accent);text-align:center}.themeMenuFoot{display:flex;align-items:center;gap:6px;margin-top:10px;padding:8px 3px 1px;border-top:1px solid #ffffff0b;color:#646983;font-size:7px}@media(max-width:800px){.themeScrim{position:fixed;inset:0;z-index:70;background:transparent}.themeMenu{right:-4px;width:min(318px,calc(100vw - 24px))}.themeButton{width:auto!important;min-width:82px!important;height:36px!important;padding:0 10px!important}.themeButtonLabel{display:inline!important;font-size:9px!important}.themeButtonDot{display:inline-block!important}.themeOption{grid-template-columns:50px 1fr 15px}.themeMode{display:none}}
/* RALLIVIO field themes v3 — distinct visual identities */
.theme-nebula{--accent:#8b7cff;--accent2:#35b8ff;--bg1:#090b1d;--bg2:#050713;--glow:#5147e8}
.theme-aurora{--accent:#19d3c5;--accent2:#3b82f6;--bg1:#06131c;--bg2:#040a12;--glow:#087f9b}
.theme-neon{--accent:#ff8a3d;--accent2:#ff4f68;--bg1:#170b09;--bg2:#0b0708;--glow:#c9472e}
.theme-lunar{--accent:#d9e7f2;--accent2:#76b9e8;--bg1:#111820;--bg2:#070b10;--glow:#5d87a8}
.theme-nebula.rv{background:radial-gradient(circle at 72% 20%,#5147e82b,transparent 30%),radial-gradient(circle at 18% 55%,#35b8ff13,transparent 34%),linear-gradient(135deg,#090b1d,#050713 62%,#0b0d20)}
.theme-aurora.rv{background:radial-gradient(circle at 70% 22%,#087f9b2b,transparent 31%),radial-gradient(circle at 15% 60%,#3b82f615,transparent 35%),linear-gradient(135deg,#06131c,#040a12 62%,#07151a)}
.theme-neon.rv{background:radial-gradient(circle at 70% 22%,#c9472e2b,transparent 30%),radial-gradient(circle at 15% 60%,#ff8a3d12,transparent 34%),linear-gradient(135deg,#170b09,#0b0708 62%,#160b0a)}
.theme-lunar.rv{background:radial-gradient(circle at 70% 22%,#5d87a822,transparent 30%),radial-gradient(circle at 15% 60%,#d9e7f20e,transparent 35%),linear-gradient(135deg,#111820,#070b10 62%,#0e151c)}
.theme-nebula .brand span,.theme-nebula .core span,.theme-nebula footer span,.theme-nebula .eyebrow,.theme-nebula .fieldHint span,.theme-nebula .signalTop span,.theme-nebula .more,.theme-aurora .brand span,.theme-aurora .core span,.theme-aurora footer span,.theme-aurora .eyebrow,.theme-aurora .fieldHint span,.theme-aurora .signalTop span,.theme-aurora .more,.theme-neon .brand span,.theme-neon .core span,.theme-neon footer span,.theme-neon .eyebrow,.theme-neon .fieldHint span,.theme-neon .signalTop span,.theme-neon .more,.theme-lunar .brand span,.theme-lunar .core span,.theme-lunar footer span,.theme-lunar .eyebrow,.theme-lunar .fieldHint span,.theme-lunar .signalTop span,.theme-lunar .more{color:var(--accent)}
.theme-nebula .topbar nav button.active,.theme-nebula .topbar nav button:hover,.theme-nebula .loginButton,.theme-nebula .round,.theme-nebula .heroSearch button,.theme-nebula .primary{background:linear-gradient(135deg,#5147e8,#35b8ff)}
.theme-aurora .topbar nav button.active,.theme-aurora .topbar nav button:hover,.theme-aurora .loginButton,.theme-aurora .round,.theme-aurora .heroSearch button,.theme-aurora .primary{background:linear-gradient(135deg,#087f9b,#19d3c5)}
.theme-neon .topbar nav button.active,.theme-neon .topbar nav button:hover,.theme-neon .loginButton,.theme-neon .round,.theme-neon .heroSearch button,.theme-neon .primary{background:linear-gradient(135deg,#c9472e,#ff8a3d)}
.theme-lunar .topbar nav button.active,.theme-lunar .topbar nav button:hover,.theme-lunar .loginButton,.theme-lunar .round,.theme-lunar .heroSearch button,.theme-lunar .primary{background:linear-gradient(135deg,#5d87a8,#d9e7f2);color:#071018}
.theme-nebula .core{background:radial-gradient(circle at 34% 24%,#5362b8 0,#222653 27%,#10132e 57%,#050713 80%);box-shadow:0 0 70px #5147e866,0 0 125px #35b8ff20,0 0 0 18px #5147e80b;border-color:#91a7ffcc}.theme-aurora .core{background:radial-gradient(circle at 34% 24%,#2d8d91 0,#123f49 27%,#09252d 57%,#040a12 80%);box-shadow:0 0 70px #087f9b66,0 0 125px #19d3c520,0 0 0 18px #087f9b0b;border-color:#63e6dfcc}.theme-neon .core{background:radial-gradient(circle at 34% 24%,#a9573d 0,#4d2119 27%,#24100d 57%,#0b0708 80%);box-shadow:0 0 70px #c9472e66,0 0 125px #ff8a3d20,0 0 0 18px #c9472e0b;border-color:#ffad83cc}.theme-lunar .core{background:radial-gradient(circle at 34% 24%,#8ba4b7 0,#3a4d5c 27%,#1b2934 57%,#070b10 80%);box-shadow:0 0 70px #5d87a866,0 0 125px #d9e7f220,0 0 0 18px #5d87a80b;border-color:#d9e7f2cc}
.theme-nebula{--accent:#a56bff;--accent2:#58d8ff;--bg1:#070a1b;--bg2:#050712;--glow:#7652ff}
.headerSearchWrap{min-width:0}
@media(max-width:1500px){.topbar{grid-template-columns:210px minmax(0,1fr) 270px auto;gap:16px!important}.topbar nav{gap:20px!important}.topbar nav button{font-size:13px!important}.topbar nav button:nth-child(3){max-width:155px}.brand{font-size:27px!important}.topbar .search{width:100%!important;min-width:0!important}.topActions{gap:8px!important}}
@media(max-width:1150px){.topbar{grid-template-columns:190px minmax(0,1fr) auto;gap:14px!important}.topbar nav{display:none!important}.topbar .search{max-width:none!important}.topActions{min-width:0!important}}
@media(max-width:800px){.topbar{display:flex!important;padding:10px 16px!important;gap:10px!important}.brand{font-size:23px!important}.topbar .search{display:none!important}.topActions{margin-left:auto}.topActions .plansButton{display:none!important}}
.theme-aurora{--accent:#42e6c0;--accent2:#6de9ff;--bg1:#061a20;--bg2:#061316;--glow:#12cfae}.theme-neon{--accent:#ff62c6;--accent2:#ff9b5e;--bg1:#1b0717;--bg2:#10070d;--glow:#ff3b9d}.theme-lunar{--accent:#a9dcff;--accent2:#e8f5ff;--bg1:#101725;--bg2:#070b12;--glow:#7dbfff}
...
.radarSection{position:relative;z-index:2;display:grid;grid-template-columns:1.05fr 1fr .92fr;gap:14px;margin:14px 5vw 35px}.radarPanel,.topicsPanel,.spotlightPanel{min-width:0;padding:17px 18px;border:1px solid rgba(85,125,205,.25);border-radius:16px;background:linear-gradient(145deg,#0b1730,#071022);box-shadow:0 18px 50px rgba(0,0,0,.2)}.radarPanelHead{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.radarPanelHead h3{margin:0;font-size:15px;color:#edf0fb}.radarPanelHead p{margin:4px 0 0;color:#7e849e;font-size:8px}.radarVisual{position:relative;height:165px;margin:8px 0 2px;overflow:hidden;border:1px solid rgba(84,191,255,.12);border-radius:50% 50% 46% 46%;background:radial-gradient(circle at 50% 50%,rgba(22,87,126,.18),rgba(4,18,35,.72) 52%,rgba(2,8,19,.95) 100%);box-shadow:inset 0 0 35px rgba(0,0,0,.65),inset 0 0 0 1px rgba(112,210,255,.04),0 10px 25px rgba(0,0,0,.2)}
.radarHud{position:absolute;z-index:9;left:12px;right:12px;top:9px;display:flex;justify-content:space-between;font:700 7px/1 system-ui;color:#5fcfff;letter-spacing:1px;text-shadow:0 0 8px #4dcaff55}.radarHud b{color:#5be6aa;font-size:6px;letter-spacing:.8px}.radarHud b:before{content:"";display:inline-block;width:4px;height:4px;margin-right:4px;border-radius:50%;background:#5be6aa;box-shadow:0 0 7px #5be6aa;animation:livePulse 1s infinite}
.radarGrid{position:absolute;inset:18px 18px 12px;border-radius:50%;background:linear-gradient(90deg,transparent 49.7%,rgba(91,204,255,.13) 50%,transparent 50.3%),linear-gradient(0deg,transparent 49.7%,rgba(91,204,255,.13) 50%,transparent 50.3%),repeating-linear-gradient(90deg,transparent 0 29px,rgba(91,204,255,.035) 30px 31px),repeating-linear-gradient(0deg,transparent 0 29px,rgba(91,204,255,.035) 30px 31px);clip-path:circle(50% at 50% 50%);opacity:.9}
.radarRings{position:absolute;left:50%;top:52%;width:126px;height:126px;transform:translate(-50%,-50%);border-radius:50%;background:repeating-radial-gradient(circle at center,transparent 0 14px,rgba(104,205,255,.14) 15px 16px,transparent 17px 30px);box-shadow:0 0 0 1px rgba(91,204,255,.28),0 0 0 31px rgba(91,204,255,.045),0 0 0 62px rgba(91,204,255,.025)}
.radarRings:before,.radarRings:after{content:"";position:absolute;inset:-1px;border-radius:50%;border:1px solid rgba(94,202,255,.16)}.radarRings:after{inset:30px;border-color:rgba(94,202,255,.18)}
.radarRings i{position:absolute;width:5px;height:5px;border-radius:50%;background:#5fd8ff;box-shadow:0 0 10px #5fd8ff}.radarRings i:nth-child(1){left:62px;top:62px}.radarRings i:nth-child(2){left:94px;top:31px;background:#ff6378;box-shadow:0 0 12px #ff6378}.radarRings i:nth-child(3){left:24px;top:78px;background:#4fc7ff}.radarRings i:nth-child(4){left:77px;top:99px;background:#5ce9ad}.radarRings i:nth-child(5){left:39px;top:35px;background:#7ee8ff}.radarRings i:nth-child(6){left:103px;top:82px;background:#ffd166}
.radarRings b{position:absolute;inset:0;border:1px solid rgba(87,202,255,.25);border-radius:50%}
.radarSweep{position:absolute;z-index:5;left:50%;top:52%;width:63px;height:2px;transform-origin:0 50%;background:linear-gradient(90deg,#67dcff,rgba(103,220,255,.35),transparent);box-shadow:0 0 10px #5fd5ff;animation:radarSweep 2.4s linear infinite}.radarSweep:after{content:"";position:absolute;left:0;top:-52px;width:105px;height:105px;transform:rotate(-24deg);transform-origin:0 100%;background:conic-gradient(from 0deg,rgba(81,210,255,.22),rgba(81,210,255,.07) 18deg,transparent 34deg);border-radius:100% 0 0 0}
.radarCrosshair{position:absolute;z-index:3;left:50%;top:52%;width:132px;height:132px;transform:translate(-50%,-50%);border-radius:50%;background:linear-gradient(90deg,transparent 49.5%,rgba(91,204,255,.13) 50%,transparent 50.5%),linear-gradient(0deg,transparent 49.5%,rgba(91,204,255,.13) 50%,transparent 50.5%);pointer-events:none}
.radarCenter{position:absolute;z-index:8;left:50%;top:52%;width:8px;height:8px;transform:translate(-50%,-50%);border:1px solid #b5f0ff;border-radius:50%;box-shadow:0 0 10px #5fd8ff;background:#16384b}.radarCenter i{position:absolute;inset:2px;border-radius:50%;background:#5ee0ff;box-shadow:0 0 8px #5ee0ff}
.radarTarget{position:absolute;z-index:7;display:flex;align-items:center;gap:3px;color:#65dfff;font:700 6px/1 system-ui;opacity:.9}.radarTarget i{width:6px;height:6px;border-radius:50%;background:#5fe0ff;box-shadow:0 0 10px #5fe0ff}.radarTarget b{font-size:5px;color:#79b8cf;font-weight:600}.radarTarget.t1{left:58%;top:38%}.radarTarget.t2{left:31%;top:62%}.radarTarget.t3{left:68%;top:69%}.radarTarget.t4{left:42%;top:30%}.radarTarget.t5{left:74%;top:49%}.radarTarget.t2 i{background:#ff6478;box-shadow:0 0 10px #ff6478}.radarTarget.t3 i{background:#5be6aa;box-shadow:0 0 10px #5be6aa}.radarTarget.t5 i{background:#ffd166;box-shadow:0 0 10px #ffd166}
.radarRange{position:absolute;z-index:8;color:#52788d;font:600 5px/1 system-ui;letter-spacing:.4px}.radarRange.r1{left:calc(50% + 9px);top:56%}.radarRange.r2{left:calc(50% + 20px);top:45%}.radarRange.r3{left:calc(50% + 32px);top:34%}
.radarVisual.radarIdle{display:grid;place-items:center}.liveRadar .radarRings{animation:radarPulse 2.4s ease-in-out infinite}.scanState{color:#6dc8ff}.scanState i{background:#5cc9ff;box-shadow:0 0 10px #5cc9ff}
@keyframes radarSweep{to{transform:rotate(360deg)}}@keyframes radarPulse{50%{transform:translate(-50%,-50%) scale(1.018);opacity:.96}}
...
.spotlightList{display:grid;margin-top:12px}.spotlightList button{display:grid;grid-template-columns:34px minmax(0,1fr) minmax(118px,auto);align-items:center;gap:9px;padding:8px 0;border:0;border-top:1px solid rgba(255,255,255,.055);background:transparent;color:#eee;text-align:left;cursor:pointer}.spotlightList img{width:34px;height:34px;border-radius:50%;object-fit:cover;border:1px solid rgba(255,255,255,.12)}.spotlightList span{min-width:0}.spotlightList b{display:block;font-size:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.spotlightList small{display:block;margin-top:3px;color:#7f849c;font-size:7px}.spotlightList em{font-style:normal;padding:6px 10px;border:1px solid #72a8ff66;border-radius:15px;color:#b9d4ff;font-size:7px}
...
.topActions{display:flex!important;align-items:center!important;justify-content:flex-end!important;gap:10px!important;flex:0 0 auto!important;min-width:max-content!important}
...
.core>strong,.core>small,.core>i{position:relative!important;z-index:9!important;text-shadow:0 2px 12px rgba(0,0,0,.72),0 0 18px rgba(0,0,0,.42)!important}.core>strong{font-size:42px!important;letter-spacing:-2.5px!important;color:#f5fbff!important}.core>strong span{color:#69dfff!important}.core>small{font-size:8px!important;color:#e5eff8!important}.core>i{font-size:8px!important;color:#74e7b7!important}.core>.coreLight{z-index:2!important}.core>.coreHalo{z-index:1!important}.globeStage{pointer-events:none!important}.globeStage,.discoverGlobeCanvas,.globeFallback{z-index:1!important}
...
.topActions .round:not(.plansButton){background:transparent!important;border:0!important;box-shadow:none!important;color:#d9f5ff!important}
.loginTextButton{min-width:82px!important;width:auto!important;padding:0 18px!important;background:linear-gradient(135deg,#7b4aff,#b45eff)!important;border:1px solid rgba(255,255,255,.2)!important;color:#fff!important}
.profileButton{width:40px!important;min-width:40px!important;height:40px!important;padding:0!important;border-radius:50%!important;overflow:hidden!important;background:linear-gradient(135deg,#7b4aff,#46dfff)!important}
.profileButton img{width:100%!important;height:100%!important;object-fit:cover!important;display:block!important}
.profileButton span{display:grid!important;place-items:center!important;width:100%!important;height:100%!important;font-size:14px!important;font-weight:900!important;color:#fff!important}
...
/* Final reference header geometry — nav and search never overlap. */
.topbar{display:grid!important;grid-template-columns:250px 500px 300px max-content!important;align-items:center!important;column-gap:20px!important;padding:0 28px!important;height:72px!important;min-height:72px!important;overflow:visible!important}
.topbar nav{grid-column:2!important;width:500px!important;min-width:500px!important;display:flex!important;align-items:center!important;justify-content:flex-start!important;gap:14px!important;overflow:visible!important;padding:0!important}
.topbar nav button{flex:0 0 auto!important;white-space:nowrap!important;font-size:13px!important}
.topbar .search{grid-column:3!important;width:300px!important;min-width:300px!important;max-width:300px!important;height:44px!important;margin:0!important;justify-self:stretch!important}
.topActions{grid-column:4!important;display:flex!important;align-items:center!important;justify-content:flex-end!important;gap:12px!important;min-width:max-content!important;margin:0!important}
@media(max-width:1550px){
 .topbar{grid-template-columns:235px 480px 290px max-content!important;column-gap:16px!important;padding:0 20px!important}
 .topbar nav{width:480px!important;min-width:480px!important;gap:12px!important}
 .topbar nav button{font-size:12px!important}
 .topbar .search{width:290px!important;min-width:290px!important;max-width:290px!important}
}
@media(max-width:1250px){
 .topbar{grid-template-columns:190px minmax(0,1fr) 270px max-content!important;column-gap:12px!important;padding:0 18px!important}
 .topbar nav{width:100%!important;min-width:0!important;gap:12px!important}
 .topbar nav button{font-size:12px!important}
 .topbar .search{width:270px!important;min-width:220px!important;max-width:270px!important}
 .themeButton{min-width:40px!important;width:40px!important;padding:0!important;justify-content:center!important}
 .themeButtonLabel,.themeButtonDot{display:none!important}
 .loginTextButton{width:76px!important;min-width:76px!important}
}
@media(max-width:950px){
 ...
 .topbar nav{order:3!important;width:100%!important;min-width:0!important;overflow:auto!important;gap:18px!important}
