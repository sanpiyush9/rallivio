  ...
  return <main className={`rv theme-${theme}`}>
    ...
    <section className="signals" id="signals">
      <div className="signalsHead">
        <div>
          <span className="eyebrow">RALLIVIO PULSE</span>
          <h2>Catch what’s moving before it becomes obvious.</h2>
          <p>The field turns live source observations into signals — what is moving, breaking out, rising, emerging, dropping now and worth watching.</p>
        </div>
        <div className="sourceState"><i/> SOURCE CONNECTED <b>YouTube</b><small>{loading ? "Synchronizing live observations…" : `${ranked.length} verified observations · refreshes every 60s`}</small></div>
      </div>

      <div className="signalTypes">{signalGroups.map(g => <button key={g.name} className={`signalType ${g.items.length ? "hasData" : "empty"}`} type="button" onClick={() => { if (g.items[0]) setModal(g.items[0]); else setNotice(`No verified ${g.name} observations are available right now.`); }}>
        <span className="signalDot"/><strong>{g.name}</strong><small>{g.items.length ? `${g.items.length} verified now` : "Awaiting signal"}</small>
      </button>)}</div>

      <div className="pulseIntro">
        <div><span className="eyebrow">LIVE SIGNAL STREAM</span><strong>Every observation has a place in the field.</strong></div>
        <span className="pulseCount">{loading ? "SYNCING" : `${ranked.length} SIGNALS`}</span>
      </div>

      <div className="signalGrid signalGridWide">
        {ranked.slice(0, 12).map((x, i) => <button className="signalCard" key={x.id} type="button" onClick={() => setModal(x)}>
          <div className="signalIcon"><img src={x.thumbnail} alt=""/></div>
          <div className="signalBody">
            <div className="signalTop"><span>{x.metadata?.signal || "Observed"}</span><time>{age(x.published_at)}</time></div>
            <strong>{x.title}</strong>
            <small>{x.channel_title}</small>
            <div className="signalMeta"><span>{fmt(x.views)} views</span><span>{categoryFor(x)}</span><span>{x.metadata?.momentum_score != null ? `Momentum ${Math.round(x.metadata.momentum_score)}` : "Verified"}</span></div>
          </div>
          <em>{i < 3 ? "↗" : "•"}</em>
        </button>)}
        {!loading && !ranked.length && <div className="signalEmpty">No verified observations are available for this environment yet.</div>}
        {loading && <div className="signalEmpty">Synchronizing the live YouTube field…</div>}
      </div>
    </section>

    <section className="discoverySurface">
      <div className="surfaceHead">
        <div><span className="eyebrow">DISCOVERY RADAR</span><h2>{filter === "Trending" ? "The field, at a glance." : `${filter} is moving`}</h2><p>Explore the live candidate pool by category, signal and momentum. Future sources can join the same field without changing this experience.</p></div>
        <button type="button" onClick={() => { setFilter("Trending"); setQ(""); }}>Reset field ↺</button>
      </div>

      <div className="radarStats">
        <div className="radarStat"><small>VERIFIED NOW</small><strong>{loading ? "—" : ranked.length}</strong><span>YouTube observations</span></div>
        <div className="radarStat"><small>TOP SIGNAL</small><strong>{ranked[0]?.metadata?.signal || "—"}</strong><span>highest current momentum</span></div>
        <div className="radarStat"><small>FASTEST CHANNEL</small><strong>{ranked[0]?.channel_title || "—"}</strong><span>current field leader</span></div>
      </div>

      <div className="radarLayout">
        <div className="cards cardsRadar">{shown.slice(0, 8).map(x => <article className="card" key={x.id} onClick={() => setModal(x)}>
          <div className="thumb"><img src={x.thumbnail} alt=""/><span>{x.metadata?.signal || categoryFor(x)}</span><button type="button" onClick={e => { e.stopPropagation(); setModal(x); }}>▶</button></div>
          <h3>{x.title}</h3><p>{x.channel_title}</p><small>{fmt(x.views)} views · {age(x.published_at)} · {categoryFor(x)}</small>
        </article>)}</div>
