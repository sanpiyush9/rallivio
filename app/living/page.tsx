...
export default function LivingDiscover() {
  ...
  const [showDiscoveryMethod, setShowDiscoveryMethod] = useState(false);
  ...
  return <main className={`rv theme-${theme}`}>
    ...
    <section className="radarSection">
      <div className="radarPanel">
        ...
        {apiUsageLatestAt && <div className="discoveryAudit">
          <button type="button" className="discoveryAuditToggle" onClick={() => setShowDiscoveryMethod(v => !v)} aria-expanded={showDiscoveryMethod}>
            <span>ⓘ How RALLIVIO detects these signals</span><b>{showDiscoveryMethod ? "Hide" : "Explain"}</b>
          </button>
          {showDiscoveryMethod && <div className="discoveryAuditBody">
            <div><b>SOURCE</b><span>YouTube public video observations collected into RALLIVIO’s verified discovery pool.</span></div>
            <div><b>WHAT IS A SIGNAL</b><span>A video observation that passes RALLIVIO’s signal validation and ranking pipeline. The signal is attached to the observed video, not a claim about the whole topic or internet.</span></div>
            <div><b>WHEN CAPTURED</b><span>Each observation carries a source statistics timestamp. The page refreshes its displayed pool every 60 seconds; new source acquisition is globally gated to avoid multiplying API usage.</span></div>
            <div><b>HOW TOPICS MOVE</b><span>Topic counts show the current number of verified signals in the pool. Topic trend movement is measured separately from repeated hourly view observations.</span></div>
            <div><b>IMPORTANT</b><span>The radar animation is a live visualization of the current verified discovery state. It does not represent geographic coordinates or fake real-time events.</span></div>
          </div>}
        </div>
        <div className={`radarVisual liveRadar ${apiUsageLatestAt ? "" : "radarIdle"}`}>{apiUsageLatestAt ? <><div className="radarHud"><span>DISCOVERY FIELD</span><b>LIVE</b></div><div className="radarGrid"/><div className="radarSweep"/><div className="radarRings"><i/><i/><i/><i/><i/><i/><b/></div><div className="radarCrosshair"/>{radarCategories.slice(0, 5).map((topic, index) => { const positions = [[57, 39], [31, 62], [68, 69], [42, 30], [74, 49]]; const [left, top] = positions[index]; const strength = Math.max(5, Math.min(12, 5 + (Number(topic.momentum || 0) / 100) * 7)); const rising = (topic.change ?? 0) >= 0; return <div key={topic.name} className={`radarTarget ${rising ? "rising" : "falling"}`} style={{ left: `${left}%`, top: `${top}%` }} title={`${topic.name} · momentum ${Math.round(topic.momentum || 0)} · ${topic.change == null ? "trend building" : `${topic.change >= 0 ? "+" : ""}${topic.change.toFixed(0)}% trend`}`}><i style={{ width: `${strength}px`, height: `${strength}px` }}/><b>{topic.name.slice(0, 3).toUpperCase()}</b></div>; })}<div className="radarCenter"><i/></div><div className="radarRange r1">25%</div><div className="radarRange r2">50%</div><div className="radarRange r3">75%</div></> : <span className="radarEmpty">No acquisition activity yet.</span>}</div>
...
.radarTarget{position:absolute;z-index:7;display:flex;align-items:center;gap:3px;color:#65dfff;font:700 6px/1 system-ui;opacity:.9;transition:left .8s ease,top .8s ease}.radarTarget.rising{color:#65e6b0}.radarTarget.falling{color:#ff7182}.radarTarget.rising i{background:#5be6aa;box-shadow:0 0 10px #5be6aa;animation:radarBlip 1.4s ease-in-out infinite}.radarTarget.falling i{background:#ff6478;box-shadow:0 0 10px #ff6478;animation:radarBlip .9s ease-in-out infinite}.radarTarget b{font-size:5px;color:#79b8cf;font-weight:600}.radarTarget i{border-radius:50%;display:block}.radarTarget i{width:6px;height:6px;border-radius:50%;background:#5fe0ff;box-shadow:0 0 10px #5fe0ff}.radarTarget b{font-size:5px;color:#79b8cf;font-weight:600}.radarTarget.t1{left:58%;top:38%}.radarTarget.t2{left:31%;top:62%}.radarTarget.t3{left:68%;top:69%}.radarTarget.t4{left:42%;top:30%}.radarTarget.t5{left:74%;top:49%}.radarTarget.t2 i{background:#ff6478;box-shadow:0 0 10px #ff6478}.radarTarget.t3 i{background:#5be6aa;box-shadow:0 0 10px #5be6aa}.radarTarget.t5 i{background:#ffd166;box-shadow:0 0 10px #ffd166}
...
@keyframes radarSweep{to{transform:rotate(360deg)}}@keyframes radarPulse{50%{transform:translate(-50%,-50%) scale(1.018);opacity:.96}}@keyframes radarBlip{50%{transform:scale(1.35);opacity:.55}}
