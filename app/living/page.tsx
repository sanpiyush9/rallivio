...
type Platform = { name: string; kind: string; connected: boolean; orbit: number; angle: number };

const platforms: Platform[] = [
  { name: "YouTube", kind: "youtube", connected: true, orbit: 1, angle: -90 },
  { name: "Instagram", kind: "instagram", connected: false, orbit: 2, angle: -40 },
  { name: "TikTok", kind: "tiktok", connected: false, orbit: 3, angle: -5 },
  { name: "X", kind: "x", connected: false, orbit: 3, angle: 35 },
  { name: "Snapchat", kind: "snapchat", connected: false, orbit: 2, angle: 75 },
  { name: "LinkedIn", kind: "linkedin", connected: false, orbit: 1, angle: 90 },
  { name: "Spotify", kind: "spotify", connected: false, orbit: 2, angle: 140 },
  { name: "Twitch", kind: "twitch", connected: false, orbit: 3, angle: 180 },
  { name: "Facebook", kind: "facebook", connected: false, orbit: 2, angle: 220 },
  { name: "Pinterest", kind: "pinterest", connected: false, orbit: 3, angle: 255 },
  { name: "Reddit", kind: "reddit", connected: false, orbit: 2, angle: 285 },
  { name: "Discord", kind: "discord", connected: false, orbit: 3, angle: 315 },
];
  ...
  return <main className="rv" onPointerMove={move} onPointerLeave={() => setPointer({ x: 0, y: 0 })}>
    ...
    <section className="hero">
      ...
      <div className="ecosystem">
        <div className={`field ${pulse ? "responding" : ""}`} aria-label="RALLIVIO living platform field">
          <div className="fieldSpace" style={{ transform: `perspective(1200px) rotateY(${pointer.x * -5}deg) rotateX(${pointer.y * 4}deg)` }}>
            ...
            {platforms.map(p => <button key={p.name} className={`platform platform-${p.orbit} ${activePlatform === p.name ? "selected" : ""}`} style={{ "--angle": `${p.angle}deg`, "--orbit": p.orbit } as CSSProperties} type="button" aria-label={`${p.name} platform`} onClick={() => activatePlatform(p)} onPointerEnter={() => setActivePlatform(p.name)} onFocus={() => setActivePlatform(p.name)}>
