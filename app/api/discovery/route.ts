...
function parseLimit(request: Request) {
  const n = Number(new URL(request.url).searchParams.get("limit") ?? "60");
  return Number.isFinite(n) ? Math.min(100, Math.max(20, Math.floor(n))) : 60;
}

function parseCursor(request: Request) {
  const n = Number(new URL(request.url).searchParams.get("cursor") ?? "0");
...
function parseTimeframe(request: Request) {
  ...
  return { id, since: new Date(Date.now() - windows[id]).toISOString() };
}
...
async function getPromotedItems() {
  ...
  const poolResponse = await supabase(
    `youtube_discovery_pool?select=id,title,channel_title,channel_id,published_at,thumbnail,description,views,likes,comments,duration,url,embeddable,live_broadcast_content,topic,format,region,metadata,acquired_at,stats_refreshed_at&id=in.(${ids.join(",")})`,
  );
  if (!poolResponse.ok) return [];
  const pool = await poolResponse.json() as PoolRow[];
  const byId = new Map(pool.map(x => [x.id, x]));
  return campaigns.map(c => {
    const item = byId.get(c.youtube_video_id);
    if (!item) return null;
    return { ...item, metadata: { ...(item.metadata ?? {}), promoted: true, promotion_campaign_id: c.id, promotion_label: "RALLIVIO Campaign", promotion_title: c.title || item.title } };
  }).filter(Boolean);
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const feedResponse = await supabase("rpc/get_discovery_timeframe_feed", {
      method: "POST",
      body: JSON.stringify({ p_since: timeframe.since, p_limit: Math.min(1000, Math.max(100, limit * 8)), p_offset: cursor, p_signal: signal, p_topic: topic && topic !== "Trending" ? topic : null }),
    });
    ...
    const items = feedRows.map(row => {
      const pool = byId.get(row.video_id);
      if (!pool) return null;
      const evidence = row.evidence ?? {};
      const metadata = {
        ...(pool.metadata ?? {}),
        signal: row.signal_type,
        signals: labels(row.signal_labels),
        momentum_score: row.momentum_score,
        signal_evidence: evidence,
      };
      const views = num(pool.views);
      const likes = num(pool.likes);
      const comments = num(pool.comments);
      return {
        id: pool.id,
        title: pool.title,
        channel_title: pool.channel_title,
        ...
        velocity: num(evidence.velocity),
        live: pool.live_broadcast_content === "live",
        url: pool.url,
        embeddable: Boolean(pool.embeddable),
        topic: pool.topic,
        region: row.region ?? pool.region,
        metadata,
        stats_refreshed_at: pool.stats_refreshed_at ?? undefined,
      };
    }).filter(Boolean);
    ...
    const topicNames = Object.entries(overview.topicCounts ?? {})
    ...
    const topicMomentumWindows: Record<string, number[]> = {};
    const topicTrendMeta: Record<string, { firstWindow: string | null; latestWindow: string | null; windows: number; firstVideos: number; latestVideos: number; minVideos: number; maxVideos: number }> = {};
    for (const name of topicNames) {
      const rows = momentumRows.filter(r => r.topic === name).sort((a, b) => Date.parse(a.window_start) - Date.parse(b.window_start)).slice(-12);
      const counts = rows.map(r => num(r.video_count));
      topicMomentumWindows[name] = rows.map(r => num(r.momentum));
      topicTrendMeta[name] = { firstWindow: rows[0]?.window_start ?? null, latestWindow: rows.at(-1)?.window_start ?? null, windows: rows.length, firstVideos: counts[0] ?? 0, latestVideos: counts.at(-1) ?? 0, minVideos: counts.length ? Math.min(...counts) : 0, maxVideos: counts.length ? Math.max(...counts) : 0 };
    }

    const nextCursor = feedRows.length >= Math.min(1000, Math.max(100, limit * 8)) ? cursor + feedRows.length : null;
    return NextResponse.json({
      ok: true,
      source: "RALLIVIO_DISCOVERY_OBSERVATIONS",
      timeframe: timeframe.id,
      refreshedAt: overview.refreshedAt ?? feedRows[0]?.observed_at ?? null,
      apiUsageLatestAt: usageRows[0]?.created_at ?? null,
      apiUsageLatestEndpoint: usageRows[0]?.endpoint ?? null,
      poolCount: num(metrics.activeVideos),
      verifiedSignalCount: num(metrics.verifiedSignals),
      trackedCreators: num(metrics.trackedCreators),
      risingCreators: num(metrics.risingCreators),
      activeTopics: num(metrics.activeTopics),
    ...
    return NextResponse.json({ ok: false, state: "DATA_UNAVAILABLE", message: error instanceof Error ? error.message : "Unknown error" }, { status: 503 });
  }
}
