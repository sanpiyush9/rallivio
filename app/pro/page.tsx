...
export default async function ProPage(){
  ...
  const {data:sub}=await supabase.from("subscriptions").select("plan_id,status,current_period_end,cancel_at_period_end").eq("user_id",user.id).maybeSingle();
  if(!sub || sub.plan_id!=="pro" || !["active","trialing"].includes(sub.status)) redirect("/pricing?required=pro");
