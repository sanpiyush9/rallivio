...
<script src="https://rallivio.com/embed.js" async></script>`,[topic,region,limit,theme]);
 return <main style={{minHeight:"100vh",padding:"48px 24px",background:"radial-gradient(circle at top,#18254d,#070b16 55%)",color:"#edf5ff",fontFamily:"Inter,system-ui,sans-serif"}}>
  <div style={{maxWidth:980,margin:"0 auto"}}>
   <div style={{fontSize:11,letterSpacing:3,color:"#66dcff",fontWeight:800}}>RALLIVIO EMBED</div>
   <h1 style={{fontSize:"clamp(34px,6vw,64px)",lineHeight:1.02,margin:"8px 0 14px"}}>Trending discovery,<br/>outside RALLIVIO.</h1>
   <p style={{maxWidth:650,color:"#9eabc4",lineHeight:1.6}}>Free for publishers. Free for creators. Show genuinely moving RALLIVIO content on your site. No signup, payment, credits or traffic exchange.</p>
   ...
   <section style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:18}}>
    ...
    <div style={card}><h2 style={h2}>Copy & paste</h2><textarea readOnly value={snippet} style={{...input,minHeight:150,fontFamily:"ui-monospace,monospace",fontSize:12}}/><button onClick={()=>navigator.clipboard.writeText(snippet)} style={button}>Copy embed code</button><p style={{color:"#8190aa",fontSize:12,lineHeight:1.5}}>The widget lazy-loads only when visible, uses a closed Shadow DOM, and fails silently if RALLIVIO is unavailable.</p></div>
   </section>
  </div>
 </main>
}
