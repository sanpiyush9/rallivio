...
export default function SiteHeader() {
  ...
  useEffect(() => { const saved = window.localStorage.getItem("rallivio-theme") as "dark" | "light" | null; const next = saved || "dark"; setTheme(next); document.documentElement.classList.toggle("light", next === "light"); }, []);
  const toggleTheme = () => { const next = theme === "dark" ? "light" : "dark"; setTheme(next); window.localStorage.setItem("rallivio-theme", next); document.documentElement.classList.toggle("light", next === "light"); };
  const submitSearch = (e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const q = search.trim(); if (q) router.push(`/creators?platform=youtube&q=${encodeURIComponent(q)}`); };
