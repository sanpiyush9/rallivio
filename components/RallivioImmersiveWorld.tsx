...
function World({items,progress,selected,onSelect}:{items:Item[];progress:number;selected:Item|null;onSelect:(x:Item)=>void}) {
  ...
  return <group ref={group}>
    ...
    {nodes.map(({item,p,s},i)=>{
      ...
      return <group key={item.id+i} position={p} scale={active?s*1.45:s} onClick={(e)=>{e.stopPropagation();onSelect(item);}}>
        ...
        {i<10&&<Html distanceFactor={8} center style={{pointerEvents:"none"}}>
          <div className="ri-node" style={{"--signal":color} as React.CSSProperties}>
    ...
    <Html position={[0,0,1.2]} center style={{pointerEvents:"none"}}>
      <div className="ri-core"><b>RALLIVIO</b><span>THE LIVING INTERNET</span></div>
    </Html>
  </group>;
}
...
export default function RallivioImmersiveWorld(){
  ...
  return <main className="ri">
    <div className="ri-stage"><Canvas dpr={[1,1.8]} camera={{position:[0,0,8.8],fov:48}} gl={{antialias:true,alpha:true}}>
    ...
    <div className="ri-vignette"/>
    <header className="ri-nav"><Link href="/" className="ri-logo">RALL<span>IVIO</span></Link><nav>{["discover","signals","creators","opportunities","about"].map((x,i)=><button key={x} onClick={()=>jump(x)}>{String(i+1).padStart(2,"0")} / {x}</button>)}</nav><button className="ri-promote" onClick={()=>setPromote(true)}>PROMOTE ↗</button></header>
    <div className="ri-progress"><i style={{transform:`scaleX(${progress})`}}/></div>

    <section id="discover" className="ri-screen ri-intro"><div className="ri-copy"><span className="ri-eyebrow">GLOBAL ATTENTION SYSTEM / LIVE</span><h1>THE INTERNET<br/><em>IS MOVING.</em></h1><p>RALLIVIO is a living map of attention — discovering movement across creators, topics, platforms and regions in real time.</p><div className="ri-actions"><button onClick={()=>jump("signals")}>ENTER THE FIELD ↓</button><button onClick={()=>setPromote(true)}>PUT CONTENT IN MOTION ↗</button></div></div><div className="ri-metrics"><b>{compact(stats.signals)}</b><span>LIVE SIGNALS</span><b>{compact(stats.creators)}</b><span>CREATOR NODES</span><b>{stats.topics}</b><span>ACTIVE WORLDS</span></div></section>

    <section id="signals" className="ri-screen ri-panel"><div><span className="ri-eyebrow">01 / SIGNAL FIELD</span><h2>Attention<br/><em>has velocity.</em></h2><p>Filter the living field. Every point is a piece of content moving through the network.</p></div><div className="ri-console"><div className="ri-search"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search the living internet"/><button onClick={()=>setPromote(true)}>PROMOTE</button></div><div className="ri-filters">{SOURCES.map(x=><button key={x} className={source===x?"on":""} onClick={()=>setSource(x)}>{x}</button>)}<i/>{SIGNALS.map(x=><button key={x} className={signal===x?"on":""} onClick={()=>setSignal(signal===x?"ALL":x)}>{x}</button>)}</div><div className="ri-stream">{visible.slice(0,6).map((x,i)=><button key={x.id} onClick={()=>setSelected(x)}><span>{String(i+1).padStart(2,"0")}</span><div><small>{x.metadata?.signal||"SIGNAL"} · {x.topic||"WORLD"}</small><b>{x.title}</b><em>{x.channel_title}</em></div><strong>{compact(x.views)}</strong></button>)}</div></div></section>

    <section id="creators" className="ri-screen ri-panel"><div><span className="ri-eyebrow">02 / CREATOR NETWORK</span><h2>People become<br/><em>nodes.</em></h2><p>{compact(stats.creators)} creators are part of the tracked attention network.</p></div><div className="ri-orbit-list">{visible.slice(0,8).map((x,i)=><button key={x.id+i} onClick={()=>setSelected(x)} style={{"--i":i} as React.CSSProperties}><img src={x.thumbnail} alt=""/><span>{x.channel_title}</span><small>{x.topic||"WORLD"} · {x.region||"GLOBAL"}</small></button>)}</div></section>

    <section id="opportunities" className="ri-screen ri-panel ri-opps"><div><span className="ri-eyebrow">03 / OPPORTUNITIES</span><h2>Where attention<br/><em>becomes action.</em></h2><p>Brands publish requirements. Creators discover opportunities. RALLIVIO connects both sides.</p><button onClick={()=>jump("about")}>EXPLORE THE NETWORK ↗</button></div><div className="ri-opportunity"><b>10%</b><span>RALLIVIO DEAL COMMISSION</span><div>BRANDS ↔ CREATORS</div></div></section>

    <section id="about" className="ri-screen ri-panel ri-about"><div><span className="ri-eyebrow">04 / RALLIVIO</span><h2>Not a page.<br/><em>A world.</em></h2><p>Discover. Promote. Connect. Create. Grow.</p></div><div className="ri-words"><span>DISCOVER</span><span>PROMOTE</span><span>CONNECT</span><span>CREATE</span><span>GROW</span></div><footer>RALLIVIO / THE LIVING INTERNET</footer></section>

    {selected&&<div className="ri-modal" onClick={()=>setSelected(null)}><div className="ri-card" onClick={e=>e.stopPropagation()}><button onClick={()=>setSelected(null)}>×</button><img src={selected.thumbnail} alt=""/><div><span>{selected.metadata?.signal||"SIGNAL"} · {selected.topic||"WORLD"} · {selected.region||"GLOBAL"}</span><h3>{selected.title}</h3><p>{selected.channel_title}</p><small>{compact(selected.views)} views · momentum {selected.metadata?.momentum_score??"—"}</small><a href={selected.url} target="_blank" rel="noreferrer">OPEN SOURCE ↗</a></div></div></div>}
    {promote&&<div className="ri-modal" onClick={()=>setPromote(false)}><div className="ri-promote-card" onClick={e=>e.stopPropagation()}><button onClick={()=>setPromote(false)}>×</button><span>RALLIVIO / PROMOTE</span><h3>Put content into the attention field.</h3><p>Paste a public content URL to begin the discovery workflow.</p><input autoFocus placeholder="https://youtube.com/... / instagram.com/..."/><button className="ri-cta" onClick={()=>setPromote(false)}>ANALYZE CONTENT ↗</button></div></div>}
  </main>;
}
