...
export default function LivingWorld({events=[]}:LivingWorldProps){
 ...
 useEffect(()=>{let disposed=false;let cleanup:(()=>void)|undefined;
  (async()=>{const THREE=await import("three");if(disposed||!ref.current)return;const host=ref.current;const scene=new THREE.Scene();const camera=new THREE.PerspectiveCamera(32,1,.1,100);camera.position.set(0,.05,4.35);const renderer=new THREE.WebGLRenderer({antialias:true,alpha:true,powerPreference:"high-performance"});renderer.setPixelRatio(Math.min(window.devicePixelRatio,1.7));renderer.setClearColor(0,0);host.appendChild(renderer.domElement);
   ...
   const texture=new THREE.CanvasTexture(canvas);const globeMaterial=globe.material as THREE.MeshPhongMaterial;globeMaterial.map=texture;globeMaterial.needsUpdate=true;
   ...
   const pointGeo=new THREE.SphereGeometry(.025,8,8);const pointMats=[0x40e8ff,0xa86dff,0x4dffb4,0xffffff];const dots:THREE.Mesh[]=[];points.forEach(([lat,lon],i)=>{const p=latLon(lat,lon,1.315);const dot=new THREE.Mesh(pointGeo,new THREE.MeshBasicMaterial({color:pointMats[i%pointMats.length]}));dot.position.set(p.x,p.y,p.z);root.add(dot);dots.push(dot)});
   const arcGroup=new THREE.Group();root.add(arcGroup);const makeArc=(a:readonly[number][],b:readonly[number][],color:number,lift:number)=>{const pa=latLon(Number(a[0]),Number(a[1]),1.31),pb=latLon(Number(b[0]),Number(b[1]),1.31);const va=new THREE.Vector3(pa.x,pa.y,pa.z),vb=new THREE.Vector3(pb.x,pb.y,pb.z);const mid=va.clone().add(vb).multiplyScalar(.5).normalize().multiplyScalar(1.31+lift);arcGroup.add(new THREE.Line(new THREE.QuadraticBezierCurve3(va,mid,vb),new THREE.LineBasicMaterial({color,transparent:true,opacity:.5})))};
   [[0,1],[1,2],[1,3],[3,4],[4,6],[5,8],[8,9],[7,10],[2,11],[6,12],[9,13]].forEach(([a,b],i)=>makeArc(points[a],points[b],i%2?0x9a63ff:0x24d8ff,.22+(i%3)*.08));
   const pulseGroup=new THREE.Group();root.add(pulseGroup);const pulseRings:THREE.Mesh[]=[];points.slice(0,9).forEach(([lat,lon],i)=>{const p=latLon(lat,lon,1.325);const ring=new THREE.Mesh(new THREE.RingGeometry(.025,.032,24),new THREE.MeshBasicMaterial({color:pointMats[i%pointMats.length],transparent:true,opacity:.75,side:THREE.DoubleSide}));ring.position.set(p.x,p.y,p.z);ring.lookAt(0,0,0);pulseGroup.add(ring);pulseRings.push(ring)});
   ...
   const clock=new THREE.Clock();let frame=0;const animate=()=>{if(disposed)return;frame=requestAnimationFrame(animate);const t=clock.getElapsedTime();root.rotation.y=t*.055;root.rotation.x=Math.sin(t*.11)*.035;dots.forEach((d,i)=>d.scale.setScalar(1+Math.sin(t*2.4+i)*.35));pulseRings.forEach((p,i)=>{const v=(t*.05+i*.17)%1;p.scale.setScalar(1+v*8);(p.material as THREE.MeshBasicMaterial).opacity=(1-v)*.55});renderer.render(scene,camera)};
   ...
   cleanup=()=>{cancelAnimationFrame(frame);ro.disconnect();renderer.dispose();texture.dispose();if(host.contains(renderer.domElement))host.removeChild(renderer.domElement);scene.traverse(o=>{const m=(o as THREE.Mesh).material;if(m&&!Array.isArray(m))m.dispose?.();const geo=(o as THREE.Mesh).geometry;geo?.dispose?.()})};
